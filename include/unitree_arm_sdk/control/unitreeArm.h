...
/*
 * Function: Set six joints commands to class lowcmd and set control gain
 * Input:    q:  joint angle
 *           qd: joint velocity
 *           tau: joint (Only used in State_LOWCMD)
 * Output:   None
 */
void setArmGainAndCmd(Vec6 kp, Vec6 kd, Vec6 q, Vec6 qd, Vec6 tau = Vec6::Zero())
{
    // covert Vec6 to std::vector<double>
    std::vector<double> kpVec(kp.data(), kp.data() + kp.size());
    std::vector<double> kdVec(kd.data(), kd.data() + kd.size());
    lowcmd->setControlGain(kpVec, kdVec);
    lowcmd->setQ(q);
    lowcmd->setQd(qd);
    lowcmd->setTau(tau);
}
...
/*
 * Function: Set Torque directly
 * Input:    tau: joint torque
 * Output:   None
 */
void setArmCmdTau(Vec6 tau)
{
    lowcmd->setZeroKd();
    lowcmd->setZeroKp();
    lowcmd->setTau(tau);
